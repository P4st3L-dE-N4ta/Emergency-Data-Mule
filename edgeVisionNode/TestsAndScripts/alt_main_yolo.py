...
Module       : Alternative Edge Vision Prototype
...
Description:
    Alternative prototype of the edge vision node. It receives GPS updates
    from a phone or logger, uses YOLO pose estimation to track a person, and
    emits posture telemetry to the Arduino serial bridge for downstream relay.
...
This script is an alternative evolution of the perception pipeline. It keeps
the same end-to-end objective as the main node: turn visual observations into
structured telemetry while testing a more advanced posture classifier.
...
class SensorLoggerHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # Read the HTTP body length and payload bytes sent by the client.
