...
Description:
    Main software responsible for coordinating the autonomous reconnaissance
    vehicle. The application integrates computer vision, GPS localization,
    motor control, and cellular communication using a multi-threaded
    architecture to detect and report potential victims in emergency
    scenarios.
...
The software is composed of four concurrent execution paths:
    ...
    Vision Thread
        - Camera acquisition and shape detection

    GPS Thread
        - GPS data acquisition

    Motor Thread
        - RC input processing and motor control

Threads communicate through shared global variables and queues.
...
GPS_PORT = '/dev/ttyACM0' # Change according to the Raspberry connection
GPS_BAUD = 9600

# Arduino MKR NB 1500 Config
#ARDUINO_PORT = '/dev/ttyACM1' # Change according to the Raspberry connection
...
def vision_thread_function():
    """
    Continuously captures frames from the camera and detects
    predefined colored geometric shapes.

    Shapes must remain visible for a configurable
    amount of time before they are considered valid and added
    to the transmission queue.

    Runs as an independent background thread.
    """
...
def gps_thread_function():
    """
    Continuously reads GPS data from the connected receiver.

    Supports both simulation mode and real hardware mode.
    The latest valid coordinates are stored in the shared
    global variable latest_gps_data.
...
def motor_thread_function():
    """
    Reads RC receiver PWM signals, performs differential drive
    calculations, and sends speed commands to the motor driver
    through I2C.

    Runs independently at approximately 20 Hz.
    """
    ...
    while running:
        ...
        # Full range goes from -128 to 127 (50 and 30 may be changed)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"Warning: Arduino bypass active (Could not connect: {e})")
